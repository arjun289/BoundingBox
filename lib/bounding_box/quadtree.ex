defmodule BoundingBox.Quadtree do
  ...
  """
  def insert(quadtree, {_lat,_lng} = child) do
    if quadtree.point == nil do
      ...
      if Map.get(quadtree, quadrant) == nil do
        Map.put(quadtree, quadrant, %Quadtree{point: child})
      else
        Map.put(quadtree, quadrant, insert(Map.get(quadtree, quadrant), child))
      end
    end
  end
  ...
  defp check_if_contains(quadtree, child, {point_lat, point_lng}) do
    ...
    case1 = point_lat > min(parent_lat, child_lat) and point_lat < max(parent_lat, child_lat)
    case2 = point_lng > min(parent_lng, child_lng) and point_lng < max(parent_lng, child_lng)
    case1 and case2
  end
